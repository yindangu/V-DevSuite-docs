---
description: >-
  V-DevSuite针对大多数通用场景已内置了一系列默认的规则、函数等插件，但实际应用中为了应对特定的需求，要对这些插件做替换或者扩展的处理。（注意：“扩展”比“替换”更加可控。）
---
...
3、版本控制变得很复杂（平台版本优先级控制、清单引用项分支优先级、项目与项目之间的优先级）

4、前期看似节省了时间，这只是表象，替换模式在后期会产生很多维护成本。

**扩展**模式就没有以上问题，唯一的缺点就是前期投入时间多一些。

**【特别说明】**针对替换覆盖平台的规则、函数的模式的版本问题 ：当项目覆盖了a规则的经过一段时间后，平台对a规则进行了改进，重新编译后时间戳就比项目的时间戳大，就又会覆盖了项目的构件，给客户的感觉系统又在反复不稳定。为了避免这个问题就会把版本号提得很高。但是接下来又修改了清单引用项分支优先级，这时又使用了平台的构件，问题还是会反反复复。然后大家就会想出很多怪招，到最后就无法维护了。所以强烈建议使用扩展现有规则函数的模式，不要使用替换模式！
